...
class VisionAgent:
    ...
    async def analyze(self, image: Image) -> ImageMetadata:
        """
        Generate descriptive metadata using Claude's vision
        """
        image_data = self.encode_image(image.path)
...
class SchemaMappingAgent:
    ...
    async def map_to_schema(
        ...
        """
        Use Claude to intelligently map data to schema fields
        """
...
IndexingAgent builds page index
    ↓
SchemaMappingAgent uses Claude + page index to map fields
    ↓
