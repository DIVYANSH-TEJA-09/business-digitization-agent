...
from openai import OpenAI  # Groq API client
from ollama import Client  # Ollama client for Qwen vision
...
def extract_business_info(
    ...
    Note:
        This function makes external API calls to Groq (gpt-oss-120b) and
        local Ollama (Qwen3.5:0.8B for vision). Network calls may take
