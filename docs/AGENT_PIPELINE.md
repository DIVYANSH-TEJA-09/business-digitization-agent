...
class VisionAgent:
    """
    Qwen3.5:0.8B-powered image analysis via Ollama
    """
    ...
    async def analyze(self, input: VisionAnalysisInput) -> ImageAnalysis:
        ...
        response = self.client.chat(
            ...
            messages=[{
                "role": "user",
                "content": prompt,
                "images": [input.image.path]
            }]
        )

        # Parse response
        analysis_data = self.parse_vision_response(response['message']['content'])
...
class SchemaMappingAgent:
    """
    LLM-powered intelligent field mapping using Groq (gpt-oss-120b)
    """
    ...
    async def classify_business_type(
        ...
        """
        Determine if business is product, service, or mixed using Groq
        """
    ...
    async def extract_products(
        ...
        """
        Extract product inventory using Groq
        """
        ...
        products_data = json.loads(response.choices[0].message.content)
